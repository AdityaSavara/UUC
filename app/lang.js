...
//the translate table
langService.table = {
	'Převést': {en: 'Convert', ae: 'Convert'},
	'Spustit (F2)': {en: 'Run (F2)', ae: 'Run (F2)'},
	'Dále': {en: 'Next', ae: 'Next'},
	'Zavřít': {en: 'Close', ae: 'Close'},
	'Dokumentace': {en: 'Documentation', ae: 'Documentation'},

	'Konstanta.': {en: 'Constant.', ae: 'Constant.'},
	'Základní, ': {en: 'Basic, ', ae: 'Basic, '},
	'prefixAll': {en: 'all prefixes can be used.', ae: 'all prefixes can be used.'},
	'prefix+': {en: 'usually only increasing prefixes are used.', ae: 'usually only increasing prefixes are used.'},
	'prefix-': {en: 'usually only decreasing prefixes are used.', ae: 'usually only decreasing prefixes are used.'},
	'prefix0': {en: 'prefixes are not used.', ae: 'prefixes are not used.'},
	'onlyUnitfuns': {en: 'Only use in {curly braces}; see tutorial', ae: 'Only use in {curly braces}; see tutorial'},

	//ERRORS 100
	'ERR_brackets_missing': {
		cz: n => `CHYBA 101: Nevyrovnané závorky, ${n} ( chybí`,
		en: n => `ERROR 101: Unbalanced brackets, ${n} ( missing`,
		ae: n => `ERROR 101: Unbalanced brackets, ${n} ( missing`
	},
	'ERR_operators': {
		cz: str => `CHYBA 102: Více operátorů vedle sebe "${str}"`,
		en: str => `ERROR 102: Several operators next to each other "${str}"`,
		ae: str => `ERROR 102: Several operators next to each other "${str}"`
	},
	'ERR_brackets_empty': {
		cz: 'CHYBA 103: Prázdné závorky ()',
		en: 'ERROR 103: Empty brackets ()',
		ae: 'ERROR 103: Empty brackets ()'
	},
	'ERR_NaN': {
		cz: str => `CHYBA 104: Nelze zpracovat číslo "${str}"`,
		en: str => `ERROR 104: Cannot parse number "${str}"`,
		ae: str => `ERROR 104: Cannot parse number "${str}"`
	},
	'ERR_unitPower': {
		cz: str => `CHYBA 105: Nelze zpracovat mocninu jednotky "${str}"`,
		en: str => `ERROR 105: Cannot parse unit power "${str}"`,
		ae: str => `ERROR 105: Cannot parse unit power "${str}"`
	},
	'ERR_unknownUnit': {
		cz: str => `CHYBA 106: Neznámá jednotka "${str}"`,
		en: str => `ERROR 106: Unknown unit "${str}"`,
		ae: str => `ERROR 106: Unknown unit "${str}"`
	},
	'ERR_operator_misplaced': {
		cz: str => `CHYBA 107: Operátor "${str}" špatně umístěn`,
		en: str => `ERROR 107: Operator "${str}" misplaced`,
		ae: str => `ERROR 107: Operator "${str}" misplaced`
	},
	'ERR_power_dim': {
		cz: 'CHYBA 108: Mocninou může být pouze bezrozměrné číslo',
		en: 'ERROR 108: Power has to be a dimensionless number',
		ae: 'ERROR 108: Power has to be a dimensionless number'
	},
	'ERR_dim_mismatch': {
		cz: 'CHYBA 109: Nesouhlasí rozměry při sčítání či odčítání',
		en: 'ERROR 109: Dimension mismatch while addition or subtraction',
		ae: 'ERROR 109: Dimension mismatch while addition or subtraction'
	},
	'ERR_special_chars': {
		cz: 'CHYBA 110: Speciální rezervované znaky # ~ nelze používat',
		en: 'ERROR 110: Special reserved characters # ~ not allowed',
		ae: 'ERROR 110: Special reserved characters # ~ not allowed'
	},
	'ERR_cbrackets_missing': {
		cz: 'CHYBA 111: Nevyrovnané složené závorky',
		en: 'ERROR 111: Unbalanced curly brackets',
		ae: 'ERROR 111: Unbalanced curly brackets'
	},
	'ERR_brackets_mismatch': {
		...
		ae: (open, close) => `ERROR 112: Mismatched brackets ${open} and ${close}`
	},
	'ERR_cbrackets_illegal': {
		...
		ae: 'ERROR 113: Incorrect use of curly brackets {}, see tutorial'
	},
	'ERR_unknown_unitfun': {
		cz: str => `CHYBA 114: Jednotka ${str} nemá k dispozici {} funkci`,
		en: str => `ERROR 114: Unit ${str} does not have a {} function`,
		ae: str => `ERROR 114: Unit ${str} does not have a {} function`
	},
	'ERR_cbrackets_dim_mismatch': {
		...
		ae: str => `ERROR 115: Dimension mismatch of input with target unit ${str}`
	},
	'ERR_NaN_result': {
		cz: 'CHYBA 116: Zakázaná matematická operace',
		en: 'ERROR 116: Illegal math operation',
		ae: 'ERROR 116: Illegal math operation'
	},
